...
int main()
{
	...
	while (n) {
		...
		while (s != "0") {
			int pos = s.find(' ');
			u = std::stoi(s.substr(0, pos));
			std::istringstream ss(s.substr(pos + 1));
			while (ss >> v) {
		...
		std::cout << std::accumulate(graph.articulationVertex.begin(), graph.articulationVertex.end(), 0) << std::endl;
